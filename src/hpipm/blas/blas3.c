...
void blasfeo_align_64_byte(void* ptr, void** ptr_align) {
    // TODO: do we need this function? can we move it somewhere else? maybe common.h?
...
// TODO visual studio alignment
#if defined(TARGET_X64_AVX2_FMA) | defined(TARGET_ARMV8A_NEON2_VPFV4)
    ALIGNED(double pU0[3 * 4 * K_MAX_STACK], 64);
#elif defined(TARGET_X64_AVX) | defined(TARGET_ARMV8A_NEON2_VPFV4)
    ...
    // algorithm scheme
    if (offsetC == 0 & offsetD == 0) {
#if defined(TARGET_X64_AVX2_FMA) | defined(TARGET_X64_AVX) | defined(TARGET_X64_SSE3) | defined(TARGET_GENERIC)
        if (m <= n) {
...
#elif defined(TARGET_GENERIC) | defined(TARGET_X64_SSE3)
    for (; jj < n - 3; jj += 4) {
...
#if defined(TARGET_X64_AVX2_FMA) | defined(TARGET_X64_AVX) | defined(TARGET_ARMV8A_NEON2_VPFV4)
left_8_m0:
    kernel_dpacp_tn_4_lib4(k, offsetA, pA + (ii + 0) * ps, sda, pU + 0 * sdu);
    kernel_dpacp_tn_4_lib4(k, offsetA, pA + (ii + 4) * ps, sda, pU + 4 * sdu);
    for (jj = 0; jj < n; jj += 4) {
...
#elif 0  // defined(TARGET_X86_SSE3)
    for (; i < m - 3; i += 4) {
        j = 0;
        for (; j < n - 1; j += 2) {
...
#elif defined(TARGET_ARMV8A_NEON2_VPFV4)
    for (; ii < m - 11; ii += 12) {
        kernel_dpacp_tn_4_lib4(k, offsetA, pA + (ii + 0) * ps, sda, pU + 0 * sdu);
        kernel_dpacp_tn_4_lib4(k, offsetA, pA + (ii + 4) * ps, sda, pU + 4 * sdu);
        kernel_dpacp_tn_4_lib4(k, offsetA, pA + (ii + 8) * ps, sda, pU + 8 * sdu);
...
#if defined(TARGET_X64_AVX) || defined(TARGET_X64_AVX2_FMA)
left_8:
    j = 0;
    for (; j < n; j += 4) {
        kernel_dtrsm_nn_ll_inv_8x4_vs_lib4(i, pA + i * sda, sda, pD + j * ps, sdd, &alpha, pB + i * sdb + j * ps, sdb, pD + i * sdd + j * ps, sdd, pA + i * sda + i * ps, sda, dA + i, m - i, n - j);
...
#if defined(TARGET_X64_AVX) || defined(TARGET_X64_AVX2_FMA) | defined(TARGET_ARMV8A_NEON2_VPFV4)
left_8:
    j = 0;
    for (; j < n; j += 4) {
        kernel_dtrsm_nn_ll_one_8x4_vs_lib4(i, pA + i * sda, sda, pD + j * ps, sdd, &alpha, pB + i * sdb + j * ps, sdb, pD + i * sdd + j * ps, sdd, pA + i * sda + i * ps, sda, m - i, n - j);
...
//	int em = m-rm;
#if 0  // defined(TARGET_X64_AVX2_FMA)
    for( ; i<m-8; i+=12)
        {
        idx = m-i; // position of already done part
        j = 0;
...
#endif
#if 0  // defined(TARGET_X64_AVX) || defined(TARGET_X64_AVX2_FMA)
    for( ; i<m-4; i+=8)
        {
        idx = m-i; // position of already done part
        j = 0;
...
void dtrsm_rlnn(int m, int n, double alpha, struct mat* sA, int ai, int aj, struct mat* sB, int bi, int bj, struct mat* sD, int di, int dj) {
#if defined(BLASFEO_REF_API)
...
void dtrmm_rutn(int m, int n, double alpha, struct mat* sB, int bi, int bj, struct mat* sA, int ai, int aj, struct mat* sD, int di, int dj) {
    // invalidate stored inverse diagonal of result matrix
    sD->use_dA = 0;
...
#elif defined(TARGET_X64_AVX) || defined(TARGET_X64_AVX2_FMA)
    for (; i < m - 7; i += 8) {
        j = 0;
        for (; j < n - 3; j += 4) {
...
void dsyr2k_ln(int m, int k, double alpha, struct mat* sA, int ai, int aj, struct mat* sB, int bi, int bj, double beta, struct mat* sC, int ci, int cj, struct mat* sD, int di, int dj) {
    double d_1 = 1.0;
    dsyrk_ln(m, k, alpha, sA, ai, aj, sB, bi, bj, beta, sC, ci, cj, sD, di, dj);
    dsyrk_ln(m, k, alpha, sB, bi, bj, sA, ai, aj, d_1, sD, di, dj, sD, di, dj);
    return;
}
...
void dsyr2k_lt(int m, int k, double alpha, struct mat* sA, int ai, int aj, struct mat* sB, int bi, int bj, double beta, struct mat* sC, int ci, int cj, struct mat* sD, int di, int dj) {
    double d_1 = 1.0;
    dsyrk_lt(m, k, alpha, sA, ai, aj, sB, bi, bj, beta, sC, ci, cj, sD, di, dj);
    dsyrk_lt(m, k, alpha, sB, bi, bj, sA, ai, aj, d_1, sD, di, dj, sD, di, dj);
    return;
}
...
void dsyr2k_un(int m, int k, double alpha, struct mat* sA, int ai, int aj, struct mat* sB, int bi, int bj, double beta, struct mat* sC, int ci, int cj, struct mat* sD, int di, int dj) {
    double d_1 = 1.0;
    dsyrk_un(m, k, alpha, sA, ai, aj, sB, bi, bj, beta, sC, ci, cj, sD, di, dj);
    dsyrk_un(m, k, alpha, sB, bi, bj, sA, ai, aj, d_1, sD, di, dj, sD, di, dj);
    return;
}
...
void dsyr2k_ut(int m, int k, double alpha, struct mat* sA, int ai, int aj, struct mat* sB, int bi, int bj, double beta, struct mat* sC, int ci, int cj, struct mat* sD, int di, int dj) {
    double d_1 = 1.0;
    dsyrk_ut(m, k, alpha, sA, ai, aj, sB, bi, bj, beta, sC, ci, cj, sD, di, dj);
    dsyrk_ut(m, k, alpha, sB, bi, bj, sA, ai, aj, d_1, sD, di, dj, sD, di, dj);
    return;
}
*/