...
hpipm_size_t d_memsize_core_qp_ipm(int nv, int ne, int nc) {
    ...
    size += 3 * nv0 * sizeof(double);  // v_bkp dv res_g
    size += 3 * ne0 * sizeof(double);  // pi_bkp dpi res_b
    size += 10 * nc0 * sizeof(double);  // lam_bkp t_bkp dlam dt res_d res_m res_m_bkp t_inv Gamma gamma
...
void d_create_core_qp_ipm(int nv, int ne, int nc, struct d_core_qp_ipm_workspace* workspace, void* mem) {
    ...
    workspace->memsize = d_memsize_core_qp_ipm(nv, ne, nc);
    ...
    if (c_ptr > ((char*) mem) + workspace->memsize) {
        printf("\nd_create_core_qp_ipm: outsize memory bounds!\n\n");
        exit(1);
    }
#endif
}
