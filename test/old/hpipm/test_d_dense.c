...
int main() {
    ...
    double H[nv * nv];
    for (ii = 0; ii < nv * nv; ii++) H[ii] = 0.0;
    for (ii = 0; ii < nv; ii++) H[ii * (nv + 1)] = 1.0;
    double g[nv];
    for (ii = 0; ii < nv; ii++) g[ii] = 10.0;
    double A[ne * nv];
    for (ii = 0; ii < ne * nv; ii++) A[ii] = 0.0;
    double b[ne];
    for (ii = 0; ii < ne; ii++) b[ii] = 0.0;
    double d_lb[nb];
    for (ii = 0; ii < nb; ii++) d_lb[ii] = -1.0;
    double d_ub[nb];
    for (ii = 0; ii < nb; ii++) d_ub[ii] = 1.0;
    int idxb[nb];
    for (ii = 0; ii < nb; ii++) idxb[ii] = ii;
    double C[0];
    double d_lg[0];
    double d_ug[0];
    double Zl[0];
    ...
    double max_res_stat;
    d_dense_qp_ipm_get_max_res_stat(&workspace, &max_res_stat);
    double max_res_eq;
    d_dense_qp_ipm_get_max_res_eq(&workspace, &max_res_eq);
    double max_res_ineq;
    d_dense_qp_ipm_get_max_res_ineq(&workspace, &max_res_ineq);
    double max_res_comp;
    d_dense_qp_ipm_get_max_res_comp(&workspace, &max_res_comp);
    double* stat;
