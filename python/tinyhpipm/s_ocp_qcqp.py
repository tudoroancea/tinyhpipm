...
class hpipm_ocp_qcqp:
    ...
    def set(
        ...
        if field.startswith("idx"):
            ...
            tmp = cast(value_cm.ctypes.data, c_int_p)
        else:
            value_cm = np.ascontiguousarray(value_cm, dtype=np.float32)
            tmp = cast(value_cm.ctypes.data, c_float_p)
        field_name_b = field.encode("utf-8")
        if idx_end is None:
            self.__hpipm.s_ocp_qcqp_set(
    ...
    def print_c_struct(self):
        self.__hpipm.s_ocp_qcqp_print(self.dim.dim_struct, self.qp_struct)

    def codegen(self, file_name: str, mode: str):
        file_name_b = file_name.encode("utf-8")
        mode_b = mode.encode("utf-8")
        self.__hpipm.s_ocp_qcqp_codegen(
...
class hpipm_ocp_qcqp_sol:
    ...
    def __get_u(self, idx_start: int, idx_end: Optional[int] = None):
        ...
        if idx_end is None:
            # get nu at stage
            tmp_ptr = cast(nu.ctypes.data, c_int_p)
            self.__hpipm.s_ocp_qcqp_dim_get_nu(self.dim.dim_struct, idx_start, tmp_ptr)
            u = np.zeros((nu[0, 0], 1))
            tmp_ptr = cast(u.ctypes.data, c_float_p)
            self.__hpipm.s_ocp_qcqp_sol_get_u(idx_start, self.qp_sol_struct, tmp_ptr)
        else:
            u = []
            for i in range(idx_start, idx_end + 1):
                # get nu at stage
                tmp_ptr = cast(nu.ctypes.data, c_int_p)
                self.__hpipm.s_ocp_qcqp_dim_get_nu(self.dim.dim_struct, i, tmp_ptr)
                u.append(np.zeros((nu[0, 0], 1)))
                tmp_ptr = cast(u[-1].ctypes.data, c_float_p)
                self.__hpipm.s_ocp_qcqp_sol_get_u(i, self.qp_sol_struct, tmp_ptr)
        return u
    ...
    def __get_x(self, idx_start: int, idx_end: Optional[int] = None):
        ...
        if idx_end is None:
            # get nx at stage
            tmp_ptr = cast(nx.ctypes.data, c_int_p)
            self.__hpipm.s_ocp_qcqp_dim_get_nx(self.dim.dim_struct, idx_start, tmp_ptr)
            x = np.zeros((nx[0, 0], 1))
            tmp_ptr = cast(x.ctypes.data, c_float_p)
            self.__hpipm.s_ocp_qcqp_sol_get_x(idx_start, self.qp_sol_struct, tmp_ptr)
        else:
            x = []
            for i in range(idx_start, idx_end + 1):
                # get nx at stage
                tmp_ptr = cast(nx.ctypes.data, c_int_p)
                self.__hpipm.s_ocp_qcqp_dim_get_nx(self.dim.dim_struct, i, tmp_ptr)
                x.append(np.zeros((nx[0, 0], 1)))
                tmp_ptr = cast(x[-1].ctypes.data, c_float_p)
                self.__hpipm.s_ocp_qcqp_sol_get_x(i, self.qp_sol_struct, tmp_ptr)
        return x

    def print_c_struct(self):
        self.__hpipm.s_ocp_qcqp_sol_print(self.dim.dim_struct, self.qp_sol_struct)
    ...
    def set(
        ...
        setter_method = {
            "u": self.__hpipm.s_ocp_qcqp_sol_set_u,
            "x": self.__hpipm.s_ocp_qcqp_sol_set_x,
            "sl": self.__hpipm.s_ocp_qcqp_sol_set_sl,
            "su": self.__hpipm.s_ocp_qcqp_sol_set_su,
        }[field]
        if idx_end is None:
            idx_end = idx_start

        for i in range(idx_start, idx_end + 1):
            value_cm = np.ascontiguousarray(value.ravel("F"), dtype=np.float32)
            tmp = cast(value_cm.ctypes.data, c_float_p)
...
class hpipm_ocp_qcqp_solver_arg:
    def __init__(self, dim: hpipm_ocp_qcqp_dim, mode: str):
        ...
        # C qp struct
        arg_struct_size = __hpipm.s_ocp_qcqp_ipm_arg_strsize()
        arg_struct = cast(create_string_buffer(arg_struct_size), c_void_p)
        self.arg_struct = arg_struct

        # C qp internal memory
        arg_mem_size = __hpipm.s_ocp_qcqp_ipm_arg_memsize(dim.dim_struct)
        arg_mem = cast(create_string_buffer(arg_mem_size), c_void_p)
        self.arg_mem = arg_mem

        # create C qp
        __hpipm.s_ocp_qcqp_ipm_arg_create(dim.dim_struct, arg_struct, arg_mem)

        # initialize default arguments
        __hpipm.s_ocp_qcqp_ipm_arg_set_default(c_mode, arg_struct)  # mode==SPEED
    ...
    def codegen(self, file_name: str, mode: str):
        file_name_b = file_name.encode("utf-8")
        mode_b = mode.encode("utf-8")
        self.__hpipm.s_ocp_qcqp_ipm_arg_codegen(
...
class hpipm_ocp_qcqp_solver:
    def __init__(self, dim: hpipm_ocp_qcqp_dim, arg: hpipm_ocp_qcqp_solver_arg):
        ...
        sizeof_s_ocp_qcqp_ipm_workspace = __hpipm.s_ocp_qcqp_ipm_ws_strsize()
        ipm_ws_struct = cast(
            create_string_buffer(sizeof_s_ocp_qcqp_ipm_workspace), c_void_p
        )
        self.ipm_ws_struct = ipm_ws_struct

        # allocate memory for ipm workspace
        ipm_size = __hpipm.s_ocp_qcqp_ipm_ws_memsize(dim.dim_struct, arg.arg_struct)
        ipm_ws_mem = cast(create_string_buffer(ipm_size), c_void_p)
        self.ipm_ws_mem = ipm_ws_mem

        # create ipm workspace
        __hpipm.s_ocp_qcqp_ipm_ws_create(
    ...
    def solve(self, qcqp: hpipm_ocp_qcqp, qp_sol: hpipm_ocp_qcqp_sol):
        self.__hpipm.s_ocp_qcqp_ipm_solve(
            qcqp.qp_struct,
    ...
    def get(self, field: str):
        if field == "stat":
            ...
            tmp = cast(iters.ctypes.data, c_int_p)
            self.__hpipm.s_ocp_qcqp_ipm_get_iter(self.ipm_ws_struct, tmp)
            # get stat_m
            stat_m = np.zeros((1, 1), dtype=int)
            tmp = cast(stat_m.ctypes.data, c_int_p)
            self.__hpipm.s_ocp_qcqp_ipm_get_stat_m(self.ipm_ws_struct, tmp)
            # get stat pointer
            res = np.zeros((iters[0][0] + 1, stat_m[0][0]))
            ptr = c_void_p()
            self.__hpipm.s_ocp_qcqp_ipm_get_stat(self.ipm_ws_struct, byref(ptr))
            tmp = cast(ptr, c_float_p)
            ...
            tmp = cast(res.ctypes.data, c_int_p)
        elif field in {"max_res_stat", "max_res_eq", "max_res_ineq", "max_res_comp"}:
            res = np.zeros((1, 1))
            tmp = cast(res.ctypes.data, c_float_p)
        else:
            raise NameError("hpipm_ocp_qcqp_solver.get: wrong field")
        field_name_b = field.encode("utf-8")
        self.__hpipm.s_ocp_qcqp_ipm_get(c_char_p(field_name_b), self.ipm_ws_struct, tmp)
        return res[0][0]
